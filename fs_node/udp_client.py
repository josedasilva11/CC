...
def request_block(file_name, block_id, server_name, server_address):
    ...
    while retries < MAX_RETRIES:
        ...
        start_time = time.time()  # Iniciar o cronômetro antes do envio da solicitação
        response, error = send_udp_request(server_name, server_address, request_data)
        end_time = time.time()
        # Calcular tempo de resposta e taxa de transferência
        response_time = end_time - start_time
        block_size = len(response['data']) if response and 'data' in response else 0
